...
def poly1(p, T, C, N, O):
  """
  Polynomial approximation to log(TOP) as a linear function in all
  variables (T,C,N,O).
  ...
  T: 1D float ndarray
     Log-temperature.
  C: 1D float ndarray
     Log-carbon abundance.
  N: 1D float ndarray
     Log-nitrogen abundance.
  O: 1D float ndarray
     Log-oxygen abundance.

  Returns
  -------
  top: 4D float ndarray
     Polynomial approximation to log(TOP) evaluated at T,C,N,O values.
  """
...
def poly4111(p, T, C, N, O):
  """
  Polynomial approximation to log(TOP) as a quartic polinomial in T,
  linear in CNO.

  See parameters and description in poly1().
  """
  mesh = np.meshgrid(C, T, N, O)
  top = (p[0]
...
def poly4141(p, T, C, N, O):
  """
  Polynomial approximation to log(TOP) as a quartic polynomial in T
  and N, linear in C and O.
...
def poly4(p, T, C, N, O):
  """
  Polynomial approximation to log(TOP) as a quartic function in all
  variables.
...
# ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
def main():
  print("\nRead TEA H2O,CO profiles over parameter space.\n"
        "For each set of [T,C,N,O] values, find the H2O-CO turnover pressure.")
  ...
  print("Fit a quartic polynomial in each of [T,C,N,O] to TOP in logspace.")
  ...
  print("\nPolynomial coefficients for TOP:\n{}".format(pfit4))


  # Show results:
  Cmesh, Tmesh, Nmesh, Omesh = np.meshgrid(C, temp, N, O)
...
if __name__ == "__main__":
  plt.ioff()
  # We will divide by zero at a few places, but don't worry,
